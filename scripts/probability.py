...
def plot_season_mlt_probability(time, trough, fin, n_season_bins=40):
    ...
    counts, xe, ye, pcm = ax.hist2d(x[trough_mask], y[trough_mask], bins=[n_season_bins, BINS['mlt']], range=[(0, 365), MLT_BOUNDS], cmap='jet')
    plt.colorbar(pcm, cax=ax, orientation='horizontal')
...
if __name__ == "__main__":
    ...
    ####################################################################################################################
    # Load trough dataset
    trough_data = np.load("E:\\ttools\\tec\\labels\\dataset.npz")
    ...
    params = {
        ...
        'newell': pandas.read_hdf("E:\\ttools\\misc\\newell.h5").values,
    ...
    plot_season_mlt_probability(trough_data['time'][kp_mask], trough[kp_mask], np.isfinite(x[kp_mask]))
    plt.show()
